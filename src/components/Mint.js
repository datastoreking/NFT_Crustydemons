...
export default function Home() {
    ...
    const connectWalletPressed = async() => {
        if (status === "No_metamask") {
            toast.warn('You must install the MetaMask extension in your browser', {
                position: "top-right",
                autoClose: 3000,
                closeOnClick: true,
                hideProgressBar: true,
    ...
    return ( <
        ...
        <
        h1 className = "freeticket" > Crusty Demons Club NFT holders are entitled to < br / > LIFETIME < span id = "freeticket" > FREE TICKETS < /span> TO CRUSTY DEMONS EVENTS</h
        1 >
        <
        div className = "home_nft" >
        <
        Image src = { home_nft }
        className = "home_nft_pic" / >
        <
        /div> <
        ...
        <
        Button className = "mint_counter_btn1"
        onClick = { reduce_Number } > - < /Button> <
        span > { mint_number } < /span> <
        Button className = "mint_counter_btn1"
        onClick = { increase_Number } > + < /Button> <
        /div > <
        Button className = "mint_counter_btn"
        value = { 5 }
        onClick = {
            (e) => setMint_number(e.target.value) } > 5 < /Button> <
        Button value = { 10 }
        onClick = {
            (e) => setMint_number(e.target.value) }
        className = "mint_counter_btn" > 10 < /Button> <
        Button value = { 30 }
        onClick = {
            (e) => setMint_number(e.target.value) }
        className = "mint_counter_btn" > 30 < /Button> <
        /div> <
        /div> <
